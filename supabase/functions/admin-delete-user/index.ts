...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!userId || typeof userId !== 'string') {
      return new Response(JSON.stringify({ error: '삭제할 회원 ID가 필요합니다.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { data: profile } = await supabaseAdmin.from('profiles').select('is_admin').eq('id', caller.id).single();

    const envBypass = Deno.env.get('WBS_ADMIN_PASSWORD') ?? '6502';
    ...
    return new Response(JSON.stringify({ error: err instanceof Error ? err.message : '서버 오류' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
