...
- **`...` 메뉴 섹션**: 조직 → 데이터(가져오기/보내기) → 설정(환경설정/단축키/권한 안내) → 관리자 기능(회원관리/변경이력) → 부분/전체 삭제(DB `is_admin` 슈퍼 관리자만, 회원 화면 미리보기 중에는 숨김).
...
2. 커밋 시 simple-git-hooks + lint-staged가 prettier + eslint --fix 적용 후 자동으로 `package.json` 패치 버전 bump + `CHANGELOG.md`에 당일 커밋 시각(`YYYY-MM-DD HH:mm`, 로컬) 섹션 추가 + `version.txt` 갱신 (pre-commit 훅, `scripts/bump-version.mjs` → `update-release.mjs`).
