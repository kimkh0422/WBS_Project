...
export interface PasteClipboardContext {
  ...
  clipboard: Task[];
  /** 붙여넣기 기준 행 — 이 행의 형제로, 이 행 바로 뒤에 삽입. null이면 표 끝/루트에 추가 */
 ...
 *
 * addTask()가 새 id를 발급하므로, 미리 만든 가짜 id로 parentId/dependencies를 지정하면
...
export function pasteClipboardTasks(ctx: PasteClipboardContext): string[] {
  const { clipboard, targetId, visibleTaskIds, tasks, addTask, updateTask } = ctx;
  if (clipboard.length === 0) return [];

  // 기준 행이 있으면 그 뒤에, 없으면 표 끝에 추가.
