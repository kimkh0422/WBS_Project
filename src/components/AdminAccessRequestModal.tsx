        ...
        {checking ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <p className="text-sm text-slate-600 leading-relaxed">
              DB에 등록된 시스템 관리자(<code className="text-xs bg-slate-100 px-1 rounded">is_admin</code>)가 아닌 경우, 여기서 권한을
              요청할 수 있습니다. 승인되면 대시보드·회원 관리 등 관리자 기능을 사용할 수 있습니다.
            </p>
            <div>
              <label htmlFor="admin-req-msg" className="block text-xs font-medium text-slate-600 mb-1.5">
                사유 (선택, 최대 500자)
              </label>
              <textarea
                id="admin-req-msg"
                ...
                className="w-full px-3 py-2.5 text-sm rounded-xl border border-[var(--color-line)] bg-slate-50 text-[var(--color-ink)] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 resize-y min-h-[88px]"
              />
            </div>
