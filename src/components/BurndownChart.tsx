...
/** 날짜별 남은 작업 수를 계산 */
function computeBurndownData(tasks: Task[], doneStatusIds: Set<string>): { date: string; ideal: number; actual: number }[] {
  // 리프 작업만 (부모 작업 제외)
...
export function BurndownChart({ tasks, projects, statusConfigs, selectedProjectId }: BurndownChartProps) {
  ...
  const doneStatusIds = useMemo(() => new Set(statusConfigs.filter((c) => c.progress === 100).map((c) => c.id)), [statusConfigs]);
  ...
  return (
    <div className="card p-6 space-y-4">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto" onMouseLeave={() => setHoveredIdx(null)}>
        ...
        {todayIdx >= 0 && (
          <>
            ...
            <text
              ...
              y={PAD.top - 5}
              textAnchor="middle"
              className="text-[9px] fill-[var(--color-accent)]"
              fontWeight={600}
            >
              오늘
            </text>
        ...
        {/* 실제 라인 */}
        {actualPath && <path d={actualPath} fill="none" stroke="#3b82f6" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" />}
