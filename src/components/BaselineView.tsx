    ...
    <div className="h-full overflow-y-auto bg-[var(--color-bg)]">
      <div className="max-w-7xl mx-auto p-6 space-y-6">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-stone-200 bg-stone-50/80">
                  ...
                  <th className="p-2 text-[10px] font-bold text-stone-500 uppercase tracking-wider w-14">ID</th>
                  <th className="p-2 text-[10px] font-bold text-stone-500 uppercase tracking-wider min-w-[200px]">작업명</th>
                  <th className="p-2 text-[10px] font-bold text-stone-500 uppercase tracking-wider w-24">베이스라인 시작</th>
                  <th className="p-2 text-[10px] font-bold text-stone-500 uppercase tracking-wider w-24">베이스라인 종료</th>
                  <th className="p-2 text-[10px] font-bold text-stone-500 uppercase tracking-wider w-24">현재 시작</th>
                  ...
                  listToShow.map((task) => {
                    ...
                    return (
                      <tr
                        ...
                        className={cn(
                          'border-b border-stone-100 hover:bg-stone-50/80 cursor-pointer transition-colors',
                      ...
                      >
                        ...
                        <td className="p-2 font-mono text-[10px] text-stone-500">{wbsId || '-'}</td>
                        <td className="p-2 font-medium text-sm text-[var(--color-ink)]" style={{ paddingLeft: `${12 + (task.depth || 0) * 20}px` }}>
                          {task.name}
                        </td>
                        <td className="p-2 text-xs font-mono text-stone-600">{fmt(task.baselineStartDate)}</td>
                        <td className="p-2 text-xs font-mono text-stone-600">{fmt(task.baselineEndDate)}</td>
                        <td className="p-2 text-xs font-mono text-stone-700">{fmt(task.startDate)}</td>
                        <td className="p-2 text-xs font-mono text-stone-700">{fmt(task.endDate)}</td>
                        <td className="p-2 text-xs font-mono text-center">
                          {startVar !== null ? (
                            <span className={cn(startVar > 0 ? 'text-red-600' : startVar < 0 ? 'text-emerald-600' : 'text-stone-500')}>
                              {startVar > 0 ? '+' : ''}{startVar}
                            </span>
                          ) : (
                            '-'
                        ...
                        <td className="p-2 text-xs font-mono text-center">
                          {endVar !== null ? (
                            <span className={cn(endVar > 0 ? 'text-red-600' : endVar < 0 ? 'text-emerald-600' : 'text-stone-500')}>
                              {endVar > 0 ? '+' : ''}{endVar}
                            </span>
                          ) : (
                            '-'
        ...
        {baselineTasksForChart.length > 0 && chartDates.days > 0 && (
          <div className="card-elevated overflow-hidden">
            <h3 className="px-4 py-3 text-sm font-bold text-stone-600 border-b border-stone-100 flex items-center gap-2">
            ...
            <div className="p-4 overflow-x-auto">
              <div className="min-w-[600px]">
                {/* 시간축 */}
                <div className="flex text-[10px] font-mono text-stone-500 border-b border-stone-200 mb-2">
                  ...
                  {baselineTasksForChart.slice(0, 30).map(task => {
                    ...
                    return (
                      <div key={task.id} className="flex items-center gap-2 h-7">
                        <div className="w-32 truncate text-xs text-stone-600 font-medium shrink-0" title={task.name}>
                          {task.name}
                        </div>
                        <div className="flex-1 h-4 bg-stone-100 rounded relative overflow-hidden min-w-[200px]">
                          <div
