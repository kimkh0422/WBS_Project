      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="새 그룹 이름"
            className="flex-1 px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
          />
          <button
            type="button"
            onClick={handleAdd}
            disabled={!newGroupName.trim()}
            className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-stone-300 disabled:cursor-not-allowed rounded-lg transition-colors"
          >
            <Plus size={14} /> 추가
          </button>
        </div>
        ...
        {groups.length === 0 ? (
          <div className="text-center py-8 text-sm text-stone-400 bg-stone-50/60 rounded-lg border border-dashed border-stone-200">
            아직 그룹이 없습니다. 위에서 새 그룹을 만들어 주세요.
          </div>
        ) : (
          <ul className="space-y-1">
            {groups.map((g, idx) => (
              <li key={g.id} className="flex items-center gap-2 px-3 py-2 bg-white border border-stone-200 rounded-lg">
                ...
                {editingId === g.id ? (
                  ...
                  <button
                    ...
                    className="flex-1 text-left text-sm text-stone-800 hover:text-indigo-600 truncate"
                    title="클릭해서 이름 변경"
                  >
                    {g.name}
                  </button>
                )}
                <span className="text-xs text-stone-400 shrink-0">{projectCountByGroup[g.id] ?? 0}개</span>
                <div className="flex items-center gap-0.5 shrink-0">
                  <button
                    type="button"
                    onClick={() => handleMove(g.id, -1)}
                    disabled={idx === 0}
                    className="p-1 text-stone-400 hover:text-stone-700 hover:bg-stone-100 disabled:opacity-30 disabled:hover:bg-transparent rounded"
                    title="위로"
                  >
                    <ChevronUp size={14} />
                  </button>
