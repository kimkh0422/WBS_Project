...
const TUTORIAL_SECTIONS: TutorialSection[] = [
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '관리자 승인 후: DB 동기화 가능. 등록된 전체 프로젝트 목록 조회, 타인 프로젝트는 멤버/권한이 있을 때만 보기·편집 가능.',
        ],
      },
      ...
      {
        type: 'paragraph',
        text: '가운데 네비게이션 탭으로 대시보드, 투입현황, 표+간트, 표만, 간트만, 칸반, 마인드맵, 프로젝트 화면을 전환합니다. 오른쪽에는 필터, DB 동기화, 더보기(⋮), 새 작업 버튼이 있습니다.',
      },
    ],
  },
  {
    ...
    content: [
      ...
      { type: 'subtitle', text: '기간·투입·주간보고' },
      {
        type: 'list',
        items: [
          '프로젝트에 시작일·종료일을 넣으면, 작업의 일정이 이 기간 밖으로 나가면 제한(검증/자동 보정)됩니다.',
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'subtitle', text: '순서·복사·삭제·일괄 수정' },
      {
        type: 'list',
        items: [
          'Alt+↑/↓: 선택한 작업의 순서를 위·아래로 이동. 드래그 앤 드롭으로 순서나 부모를 바꿀 수 있습니다.',
          'Ctrl+C / Ctrl+V: 선택한 작업(및 하위) 복사·붙여넣기. Ctrl+A로 전체 선택 후 일괄 복사 가능.',
          'Delete: 선택한 작업 삭제(하위 포함 여부는 확인 창에서 선택).',
          '여러 작업을 선택한 뒤 표 위에 나타나는 일괄 수정 바에서 상태·담당자·공수·진척률 등을 한 번에 바꿀 수 있습니다.',
        ],
      },
      ...
      {
        type: 'paragraph',
        text: '작업을 마일스톤 또는 이슈로 표시할 수 있고, 베이스라인(기준 일정·공수)을 저장해 두면 현재와 차이를 비교할 수 있습니다. 표 편집 모드를 켜면 셀 클릭만으로 해당 컬럼을 바로 수정할 수 있으며(엑셀형), 줄간격(행 높이)은 표 위 슬라이더로 15~64px 범위에서 조정할 수 있습니다.',
      },
    ],
  },
  {
    ...
    content: [
      ...
      },
      {
        ...
        items: [
          ...
          '표에서 컬럼 헤더를 클릭하면 해당 컬럼 기준으로 정렬(WBS·이름·일정·공수·담당자·상태·진척률 등).',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    content: [
      {
        type: 'paragraph',
        text: '관리자 승인을 받은 사용자만 DB 동기화를 사용할 수 있습니다. 미승인 사용자는 로컬에서만 사용 가능하며, "승인 후 서버와 동기화할 수 있다"는 안내가 표시됩니다.',
      },
      {
        type: 'list',
        items: [
          'Ctrl+S 또는 상단 "DB 동기화" 버튼을 누르면 로컬 데이터와 서버(Supabase) 데이터를 맞춥니다.',
          '동기화 시 업로드(로컬→서버)와 다운로드(서버→로컬)가 함께 이루어지며, 진행 단계가 퍼센트로 표시됩니다.',
          '충돌이 있으면 안내에 따라 업로드/다운로드 중 선택하거나, 프로젝트별로 처리할 수 있습니다. 로컬에 저장되지 않은 변경이 있으면 버튼에 "미저장" 표시가 나옵니다.',
        ],
      },
    ],
  },
  ...
  },
  {
    ...
    content: [
      ...
      {
        type: 'paragraph',
        text: '상단 ⋮ → "단축키"를 누르면 전체 목록을 패널로 볼 수 있습니다. 입력창(텍스트·날짜 등)에 포커스가 없을 때 동작합니다.',
      },
    ],
  },
  {
