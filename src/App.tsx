...
function WBSApp() {
  ...
  const handleExport = () => {
    const fileName = currentProject
      ? `wbs_${currentProject.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`
      : `wbs_export_${new Date().toISOString().split('T')[0]}.xlsx`;
              ...
              {isImportMenuOpen && (
                <>
                  ...
                  <div className="absolute top-full right-0 mt-1.5 w-48 bg-white rounded-xl shadow-xl border border-[var(--color-line)] overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <button onClick={handleImportBackupClick} className="w-full text-left px-4 py-2.5 text-xs text-[var(--color-accent)] hover:bg-blue-50 transition-colors border-t border-[var(--color-line)]">
                      전체 백업 복원 (JSON)
                    </button>
                  </div>
                </>
