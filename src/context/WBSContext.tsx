...
export function WBSProvider({ children }: { children: React.ReactNode }) {
  ...
  const addProject = (name: string, description?: string, startDate?: string) => {
    const newProject: Project = { id: uuidv4(), name, description, startDate };
      ...
      if (project && updates.startDate && updates.startDate !== project.startDate) {
        ...
        if (diff !== 0) {
          ...
          setAllTasks(currentTasks => {
            const shifted = currentTasks.map(t => t.projectId === id ? {
              ...t,
  ...
  const updateTask = (id: string, updates: Partial<Task>) => {
    ...
    setAllTasks(prev => {
      ...
      const affectsRollup = ['startDate', 'endDate', 'workEffort'].some(k => Object.prototype.hasOwnProperty.call(updates, k));
      ...
      const sortOrder = result.indexOf(updatedTask);
      upsertTask(updatedTask, sortOrder >= 0 ? sortOrder : 0).catch(err => console.error('[DB] 작업 수정 실패:', err));
      return result;
    });
  };
  ...
  const deleteAllTasks = () => {
    ...
    setAllTasks(prev => {
      ...
      if (effectiveProjectId) deleteAllTasksFromDB(effectiveProjectId).catch(err => console.error('[DB] 전체 삭제 실패:', err));
      return result;
    });
  };
  ...
  const mergeBackups = (backups: BackupData[]): { addedProjects: number; addedTasks: number } => {
    ...
    Promise.all([...newProjects.map(p => upsertProject(p)), upsertTasks(newTasks)]).catch(err => console.error('[DB] 병합 실패:', err));
...
export function useWBS() {
  ...
  if (!context) throw new Error('useWBS must be used within a WBSProvider');
  return context;
}
