...
export function useFileImportExport(deps: FileImportExportDeps) {
  ...
  const handleExportFromModal = useCallback((params: { scope: ExportScope; formats: ExportFormat[]; projectIds: string[] }) => {
  ...
  }, [projects, allTasks, wbsMap, pushToast, exportFullBackup, setLastExportPrefs]);
  ...
  const handleImportBackupClick = useCallback(() => { backupInputRef.current?.click(); }, []);
  const handleMergeImportClick = useCallback(() => { mergeInputRef.current?.click(); }, []);

  const importFromExcelFiles = useCallback(async (files: File[]) => {
  ...
  const importFromBackupJsonFiles = useCallback(async (files: File[]) => {
    if (files.length === 1) {
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (firstExt === 'xlsx' || firstExt === 'xls' || firstExt === 'xlsm') {
        ...
        setErrorAlert({ isOpen: true, message: 'Markdown(.md) 파일 가져오기는 아직 지원되지 않습니다. Excel(.xlsx) 또는 백업 JSON(.json) 파일을 선택해주세요.' });
      } else {
        setErrorAlert({ isOpen: true, message: '지원하지 않는 파일 형식입니다. Excel(.xlsx) 또는 백업 JSON(.json) 파일만 선택할 수 있습니다.' });
      }
    } finally {
  ...
  const handleMergeFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const executeImport = useCallback(async (targetProjectId: string, newProjectName?: string) => {
    try {
  ...
  const executeRestoreBackupIntoProject = useCallback(async (targetProjectId: string) => {
  ...
  return {
    ...
    handleExportFromModal, handleQuickExport,
    handleImportClick, handleImportBackupClick, handleMergeImportClick,
