#!/usr/bin/env node
/**
 * 개인 To-Do(칸반) 테이블(personal_todos) + RLS 정책(SQL)을 클립보드에 복사하고
 * Supabase SQL 에디터를 엽니다. 브라우저에서 붙여넣기(Ctrl+V) 후 Run 실행하세요.
 */
...
const sqlPath = join(__dirname, '../supabase/migrations/20260609120000_add_personal_todos.sql');
...
try {
  ...
  console.log('[완료] 개인 To-Do(personal_todos) 마이그레이션 SQL이 클립보드에 복사되었습니다.');
