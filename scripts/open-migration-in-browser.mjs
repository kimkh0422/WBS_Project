#!/usr/bin/env node
/**
 * 새 Supabase 프로젝트 전체 설정 SQL을 클립보드에 복사하고 Supabase SQL 에디터를 엽니다.
 * .env의 VITE_SUPABASE_URL에서 프로젝트 ID를 읽습니다.
 * 브라우저에서 붙여넣기(Ctrl+V) 후 Run 실행하세요.
 */
...
const sqlPath = join(__dirname, '../supabase/migrations/FULL_SETUP_NEW_PROJECT.sql');
...
try {
  ...
  console.log('[완료] 전체 설정 SQL이 클립보드에 복사되었습니다.');
  console.log('[안내] Supabase SQL 에디터를 엽니다. Ctrl+V로 붙여넣기 후 Run을 누르세요.');
  if (!projectRef) {
    console.log('[주의] .env에 VITE_SUPABASE_URL이 없습니다. 새 프로젝트 생성 후 .env를 업데이트하세요.');
  }
